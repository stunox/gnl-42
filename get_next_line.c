...
/*   Updated: 2024/11/27 13:04:01 by nbasheer         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	*line = add_b_str(*line, add_b_str(ft_substr(buf, 0, nb[0]), "\n", 0), 1);
	free(*temp);
	if (nb[0] == nb[1] - 1)
		*temp = NULL;
	else
...
static int	get_line(char *buf, char **temp, char **line, int n)
{
	...
	if (i[0] < i[1] && (*line)[i[0]] == '\n')
	{
		*temp = add_b_str(ft_strdup(&(*line)[i[0] + 1]), *temp, 1);
		(*line)[i[0] + 1] = '\0';
		return (1);
	}
